...
export class Pen {
  ...
  constructor(private ctx: CanvasRenderingContext2D, private data: IPalette) {
    this.ctx = ctx;
    this.data = data;
    this.style = {
      width: 0,
  ...
  _border({
    ...
    if (borderRadius) {
      if (typeof borderRadius === 'string') {
        ...
        if (border.length === 4) {
          r1 = Math.min(toPx(border[0], minSize), width / 2, height / 2);
    ...
    const lineWidth = borderWidth ? toPx(borderWidth, minSize) : 0;
   ...
   */
  _doClip(borderRadius: string | number, width: number, height: number, borderStyle?: string) {
  ...
  _preProcess(view: InnerView, notClip?: Boolean) {
    ...
    switch (view.type) {
      case 'text': {
        ...
        const fontStyle =
          // @ts-ignore
          view.css.fontStyle || view.css.textStyle === 'italic' || view.css.textStyle === 'oblique'
            ? view.css.textStyle
            : 'normal' || 'normal';
        ...
        this.ctx.font = `${fontStyle} ${fontWeight} ${fontSize}px "${view.css.fontFamily || 'sans-serif'}"`;
        // 计算行数
        let lines = 0;
        const linesArray = [];
        for (let i = 0; i < textArray.length; ++i) {
          const textLength = this.ctx.measureText(textArray[i]).width;
          const minWidth = fontSize + paddings[1] + paddings[3];
          let partWidth = view.css.width
            ? toPx(view.css.width, this.style.width) - paddings[1] - paddings[3]
            : textLength;
        ...
        lines = typeof view.css.maxLines === 'number' && view.css.maxLines < lines ? view.css.maxLines : lines;
      ...
      case 'image': {
        ...
        const ratio = injection.getRatio(); // getApp().systemInfo.pixelRatio ? getApp().systemInfo.pixelRatio : 2;
        ...
        if (!view.css || (view.css.width === 'auto' && view.css.height === 'auto')) {
          ...
          height = toPx(view.css.height!, this.style.height);
          width = (view.sWidth! / view.sHeight!) * height;
        } else if (view.css.height === 'auto') {
          width = toPx(view.css.width!, this.style.width);
    ...
    if (view.css && view.css.right) {
      if (typeof view.css.right === 'string') {
        x = this.style.width - toPx(view.css.right, this.style.width);
      } else {
        // 可以用数组方式，把文字长度计算进去
        // [right, 文字id, 乘数（默认 1）]
    ...
    if (view.css && view.css.bottom) {
      y = this.style.height - height - toPx(view.css.bottom, this.style.height);
    } else {
    ...
    if (view.type === 'text') {
      view.rect.minWidth = toPx(view.css.fontSize!) + paddings[1] + paddings[3];
    }
  ...
  _doBackground(view: IView) {
    ...
    this._doClip(view.css.borderRadius!, width, height, view.css.borderStyle);
    if (isGradient(String(background))) {
      doGradient(String(background), width, height, this.ctx);
    } else {
  ...
  _drawQRCode(view: IView) {
    ...
    QRapi.draw(
      ...
      width,
      height,
      view.css.background ? String(view.css.background) : '',
      view.css.color!,
    );
    this.ctx.restore();
  ...
  async _drawAbsImage(view: InnerView) {
    if (!view.url) {
      return;
    }
    this.ctx.save();
  ...
  _fillAbsText(view: InnerView) {
    ...
    this.ctx.save();
    const { width, height, extra } = this._preProcess(view, Boolean(view.css.background && view.css.borderRadius))!;
    this.ctx.fillStyle = view.css.color || 'black';
    if (view.id && penCache.textLines[view.id]) {
      this.ctx.textAlign = view.css.textAlign ? (view.css.textAlign as CanvasTextAlign) : 'left';
      for (const i of penCache.textLines[view.id]!) {
        ...
        if (textDecoration) {
          ...
          this.ctx.beginPath();
          this.ctx.moveTo(...(textDecoration.moveTo as [number, number]));
          this.ctx.lineTo(...(textDecoration.lineTo as [number, number]));
          this.ctx.closePath();
      ...
      for (let j = 0; j < textArray.length; ++j) {
        ...
        for (let i = 0; i < linesArray[j]; ++i) {
          ...
          const y = -(height / 2) + (lineIndex === 0 ? fontSize : fontSize + lineIndex * lineHeight);
          lineIndex++;
          if (view.css.textStyle === 'stroke') {
            this.ctx.strokeText(text, x, y, measuredWith);
          } else {
          ...
          let textDecoration;
          if (view.css.textDecoration && typeof view.css.textDecoration === 'string') {
            this.ctx.lineWidth = fontSize / 13;
            this.ctx.beginPath();
  ...
  _drawAbsRect(view: InnerView) {
    this.ctx.save();
    const { width, height } = this._preProcess(view)!;
    if (isGradient(view.css.color!)) {
      doGradient(view.css.color!, width, height, this.ctx);
    } else {
  ...
  _doShadow(view: InnerView) {
    // @ts-ignore
    if (!view.css || (!view.css.shadow && !view.css.boxShadow)) {
      return;
    }
    // @ts-ignore
    const _shadow = view.css.boxShadow || view.css.shadow;
    const box = _shadow.replace(/,\s+/g, ',').split(/\s+/);
    if (box.length > 4) {
