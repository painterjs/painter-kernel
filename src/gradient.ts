export function isGradient(bg: string): boolean {
  if (bg && (bg.startsWith('linear') || bg.startsWith('radial'))) {
    return true;
  }
  return false;
}

export function doGradient(bg: string, width: number, height: number, ctx: CanvasRenderingContext2D) {
  if (bg.startsWith('linear')) {
    linearEffect(width, height, bg, ctx);
  } else if (bg.startsWith('radial')) {
    radialEffect(width, height, bg, ctx);
  }
}

function analizeGrad(string: string) {
  const colorPercents = string.substring(0, string.length - 1).split('%,');
  const colors = [];
  const percents = [];
  for (let colorPercent of colorPercents) {
    colors.push(colorPercent.substring(0, colorPercent.lastIndexOf(' ')).trim());
    percents.push(parseInt(colorPercent.slice(colorPercent.lastIndexOf(' '))) / 100);
  }
  return { colors: colors, percents: percents };
}

function radialEffect(width: number, height: number, bg: string, ctx: CanvasRenderingContext2D) {
  const matched = bg.match(/radial-gradient\((.+)\)/);
  if (matched) {
    const colorPer = analizeGrad(matched[1]);
    const grd = ctx.createRadialGradient(0, 0, 0, 0, 0, width < height ? height / 2 : width / 2);
    for (let i = 0; i < colorPer.colors.length; i++) {
      grd.addColorStop(colorPer.percents[i], colorPer.colors[i]);
    }
    ctx.fillStyle = grd;
  }
}

function analizeLinear(bg: string, width: number, height: number) {
  const direction = bg.match(/([-]?\d{1,3})deg/);
  const dir = direction && direction[1] ? parseFloat(direction[1]) : 0;
  let coordinate;
  switch (dir) {
    case 0:
      coordinate = [0, -height / 2, 0, height / 2];
      break;
    case 90:
      coordinate = [width / 2, 0, -width / 2, 0];
      break;
    case -90:
      coordinate = [-width / 2, 0, width / 2, 0];
      break;
    case 180:
      coordinate = [0, height / 2, 0, -height / 2];
      break;
    case -180:
      coordinate = [0, -height / 2, 0, height / 2];
      break;
    default:
      let x1 = 0;
      let y1 = 0;
      let x2 = 0;
      let y2 = 0;
      if (dir > 0 && dir < 90) {
        x1 =
          width / 2 -
          (((width / 2) * Math.tan(((90 - dir) * Math.PI * 2) / 360) - height / 2) *
            Math.sin((2 * (90 - dir) * Math.PI * 2) / 360)) /
            2;
        y2 = Math.tan(((90 - dir) * Math.PI * 2) / 360) * x1;
        x2 = -x1;
        y1 = -y2;
      } else if (dir > -180 && dir < -90) {
        x1 =
          -(width / 2) +
          (((width / 2) * Math.tan(((90 - dir) * Math.PI * 2) / 360) - height / 2) *
            Math.sin((2 * (90 - dir) * Math.PI * 2) / 360)) /
            2;
        y2 = Math.tan(((90 - dir) * Math.PI * 2) / 360) * x1;
        x2 = -x1;
        y1 = -y2;
      } else if (dir > 90 && dir < 180) {
        x1 =
          width / 2 +
          ((-(width / 2) * Math.tan(((90 - dir) * Math.PI * 2) / 360) - height / 2) *
            Math.sin((2 * (90 - dir) * Math.PI * 2) / 360)) /
            2;
        y2 = Math.tan(((90 - dir) * Math.PI * 2) / 360) * x1;
        x2 = -x1;
        y1 = -y2;
      } else {
        x1 =
          -(width / 2) -
          ((-(width / 2) * Math.tan(((90 - dir) * Math.PI * 2) / 360) - height / 2) *
            Math.sin((2 * (90 - dir) * Math.PI * 2) / 360)) /
            2;
        y2 = Math.tan(((90 - dir) * Math.PI * 2) / 360) * x1;
        x2 = -x1;
        y1 = -y2;
      }
      coordinate = [x1, y1, x2, y2];
      break;
  }
  return coordinate;
}

function linearEffect(width: number, height: number, bg: string, ctx: CanvasRenderingContext2D) {
  const param = analizeLinear(bg, width, height);
  const grd = ctx.createLinearGradient(param[0], param[1], param[2], param[3]);
  const matched = bg.match(/linear-gradient\((.+)\)/);
  if (matched) {
    const content = matched[1];
    const colorPer = analizeGrad(content.substring(content.indexOf(',') + 1));
    for (let i = 0; i < colorPer.colors.length; i++) {
      grd.addColorStop(colorPer.percents[i], colorPer.colors[i]);
    }
    ctx.fillStyle = grd;
  }
}
